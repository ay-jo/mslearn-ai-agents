...
# NOTE: Semantic kernel was replaced by agent framework in fourndry SDK
...
async def process_expenses_data(prompt, expenses_data):
    ...
    async with (
        ...
        expenses_agent_def = await project_client.agents.create_agent(
                ...
                name = "expenses_agent",
                instructions="""You are an AI assistant for expense clain submission.
                                When a user submits expenses data and requests an expense claim, use the plugin.  Then confirm to the user that you've done so."""
        ...
        # created and returned with initial response
